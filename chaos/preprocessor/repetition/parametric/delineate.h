...
#
# if CHAOS_PP_VARIADICS
#    define CHAOS_PP_DELINEATE_PARAMETRIC_S(s, size, count, sep, macro, ...) \
        CHAOS_PP_EXPR_S(s)(CHAOS_PP_PARAMETRIC_S( \
            s, CHAOS_IP_DELINEATE_PARAMETRIC_INDIRECT, size, \
            CHAOS_PP_NEXT(s), count, CHAOS_PP_EMPTY, sep, macro, CHAOS_PP_TRAMPOLINE(macro), __VA_ARGS__ \
        )) \
        /**/
...
#    define CHAOS_IP_DELINEATE_PARAMETRIC_I(s, id, para, o, count, s1, s2, macro, _m, ...) \
        CHAOS_PP_IF(count)( \
            CHAOS_IP_DELINEATE_PARAMETRIC_II, CHAOS_PP_TUPLE_EAT(?) \
        )(CHAOS_PP_OBSTRUCT(), s, id, para, o, CHAOS_PP_DEC(count), s1, s2, macro, _m, __VA_ARGS__) \
        /**/
#    define CHAOS_IP_DELINEATE_PARAMETRIC_II(_, s, id, para, o, count, s1, s2, macro, _m, ...) \
