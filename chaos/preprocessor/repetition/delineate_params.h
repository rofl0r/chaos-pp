...
# define CHAOS_IP_DELINEATE_PARAMS_I(_, s, id, count, sep, ss, pp) \
    CHAOS_PP_EXPR_IF _(count)( \
        ...
        CHAOS_PP_PRIMITIVE_CAT(CHAOS_IP_DELINEATE_PARAMS_, id) _(CHAOS_PP_DEC(count), CHAOS_PP_UNPACK pp) sep _() \
    ) \
    /**/
#
# if CHAOS_PP_VARIADICS
#    define CHAOS_IP_DELINEATE_PARAMS_0(c, ...) __VA_ARGS__ ## c
#    define CHAOS_IP_DELINEATE_PARAMS_1(c, ...) CHAOS_PP_DEFER(CHAOS_IP_DELINEATE_PARAMS_2)(c, CHAOS_PP_TUPLE_REM(?) __VA_ARGS__)
