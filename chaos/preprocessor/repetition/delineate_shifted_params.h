...
#
# if CHAOS_PP_VARIADICS
#    define CHAOS_PP_DELINEATE_SHIFTED_PARAMS_BYPASS(s, count, sep, ...) \
        CHAOS_PP_EXPR_S(s)(CHAOS_IP_DELINEATE_SHIFTED_PARAMS_I( \
            CHAOS_PP_OBSTRUCT(), CHAOS_PP_PREV(s), CHAOS_PP_IS_VARIADIC(__VA_ARGS__), CHAOS_PP_DEC(count), CHAOS_PP_EMPTY, sep, (__VA_ARGS__) \
        )) \
        /**/
#    define CHAOS_PP_DELINEATE_SHIFTED_PARAMS_BYPASS_ CHAOS_PP_LAMBDA(CHAOS_PP_DELINEATE_SHIFTED_PARAMS_BYPASS_ID)()
# else
#    define CHAOS_PP_DELINEATE_SHIFTED_PARAMS_BYPASS(s, count, sep, param) \
        CHAOS_PP_EXPR_S(s)(CHAOS_IP_DELINEATE_SHIFTED_PARAMS_I( \
            CHAOS_PP_OBSTRUCT(), CHAOS_PP_PREV(s), CHAOS_PP_IS_BINARY(param), CHAOS_PP_DEC(count), CHAOS_PP_EMPTY, sep, (param) \
        )) \
        /**/
# endif
#
...
#    define CHAOS_IP_DELINEATE_SHIFTED_PARAMS_1(c, ...) CHAOS_PP_DEFER(CHAOS_IP_DELINEATE_SHIFTED_PARAMS_2)(c, CHAOS_PP_TUPLE_REM(?) __VA_ARGS__)
