...
#    define CHAOS_PP_FOR_EACH_PARAMETRIC(size, macro, ...) CHAOS_PP_FOR_EACH_PARAMETRIC_S(CHAOS_PP_STATE(), size, macro, __VA_ARGS__)
...
#
# if CHAOS_PP_VARIADICS
#    define CHAOS_PP_FOR_EACH_PARAMETRIC_S(s, size, macro, ...) \
        CHAOS_IP_FOR_EACH_PARAMETRIC_I(s, size, macro, CHAOS_PP_NON_OPTIONAL(__VA_ARGS__), CHAOS_PP_EXPOSE(CHAOS_PP_PACK_OPTIONAL(__VA_ARGS__))) \
        /**/
...
#    define CHAOS_IP_FOR_EACH_PARAMETRIC_I(s, size, macro, g, im) \
        CHAOS_PP_EXPR_S(s)(CHAOS_PP_PARAMETRIC_S( \
            s, CHAOS_IP_FOR_EACH_PARAMETRIC_INDIRECT, size, \
            CHAOS_PP_NEXT(s), macro, CHAOS_PP_TRAMPOLINE(macro), CHAOS_PP_TYPEOF(g), g, im \
        )) \
        /**/
#    define CHAOS_IP_FOR_EACH_PARAMETRIC_INDIRECT() CHAOS_IP_FOR_EACH_PARAMETRIC_II
#    define CHAOS_IP_FOR_EACH_PARAMETRIC_II(s, id, para, o, macro, _m, type, g, ...) \
        CHAOS_PP_IIF(CHAOS_PP_IS_CONS(g))( \
            CHAOS_IP_FOR_EACH_PARAMETRIC_III, CHAOS_PP_TUPLE_EAT(?) \
        )(CHAOS_PP_OBSTRUCT(), s, id, para, o, macro, _m, type, g, __VA_ARGS__) \
        /**/
#    define CHAOS_IP_FOR_EACH_PARAMETRIC_III(_, s, id, para, o, macro, _m, type, g, ...) \
        _m()(o, macro, CHAOS_PP_ITEM(type, CHAOS_PP_FIRST(g)) __VA_ARGS__) \
