...
#
# if CHAOS_PP_VARIADICS
#    define CHAOS_PP_DELINEATE_PARAMETRIC_S(s, size, count, sep, macro, ...) CHAOS_IP_DELINEATE_PARAMETRIC_U(s, size, count, sep, macro, (__VA_ARGS__))
#    define CHAOS_PP_DELINEATE_PARAMETRIC_S_ CHAOS_PP_LAMBDA(CHAOS_PP_DELINEATE_PARAMETRIC_S_ID)()
# else
#    define CHAOS_PP_DELINEATE_PARAMETRIC_S(s, size, count, sep, macro, data) CHAOS_IP_DELINEATE_PARAMETRIC_U(s, size, count, sep, macro, (data))
# endif
#
# define CHAOS_PP_DELINEATE_PARAMETRIC_S_ID() CHAOS_PP_DELINEATE_PARAMETRIC_S
#
# define CHAOS_IP_DELINEATE_PARAMETRIC_U(s, size, count, sep, macro, pd) \
    CHAOS_PP_EXPR_S(s)(CHAOS_IP_DELINEATE_PARAMETRIC_I( \
        CHAOS_PP_NEXT(s), CHAOS_PP_BUFFER(s, size), count, sep, macro, pd \
    )) \
    /**/
# define CHAOS_IP_DELINEATE_PARAMETRIC_I(s, buffer, count, sep, macro, pd) \
    CHAOS_IP_DELINEATE_PARAMETRIC_II(s, s, buffer, buffer, count, CHAOS_PP_EMPTY, sep, macro, CHAOS_PP_TRAMPOLINE(macro), pd) \
    /**/
# define CHAOS_IP_DELINEATE_PARAMETRIC_INDIRECT() CHAOS_IP_DELINEATE_PARAMETRIC_II
# define CHAOS_IP_DELINEATE_PARAMETRIC_II(s, o, buffer, shelf, count, sep, ss, macro, _m, pd) \
    CHAOS_PP_IIF(CHAOS_PP_BITAND(CHAOS_PP_IS_VALID(buffer))(CHAOS_PP_NOT_EQUAL(buffer, CHAOS_PP_LIMIT_EXPR)))( \
        CHAOS_PP_IF(count)( \
            CHAOS_IP_DELINEATE_PARAMETRIC_III, CHAOS_PP_TUPLE_EAT(10) \
        ), \
        CHAOS_IP_DELINEATE_PARAMETRIC_P \
    )(s, o, buffer, shelf, count, sep, ss, macro, _m, pd) \
    /**/
# define CHAOS_IP_DELINEATE_PARAMETRIC_III(s, o, buffer, shelf, count, sep, ss, macro, _m, pd) \
    ...
    /**/
# define CHAOS_IP_DELINEATE_PARAMETRIC_P(s, o, buffer, shelf, count, sep, ss, macro, _m, pd) \
