...
#    define CHAOS_PP_FOR_EACH_I_PARAMETRIC(size, macro, g, ...) CHAOS_PP_FOR_EACH_I_PARAMETRIC_S(CHAOS_PP_STATE(), size, macro, g, __VA_ARGS__)
...
#
# if CHAOS_PP_VARIADICS
#    define CHAOS_PP_FOR_EACH_I_PARAMETRIC_S(s, size, macro, g, ...) \
        CHAOS_PP_EXPR_S(s)(CHAOS_PP_PARAMETRIC_S( \
            s, CHAOS_IP_FOR_EACH_I_PARAMETRIC_INDIRECT, size, \
            CHAOS_PP_NEXT(s), 0, macro, CHAOS_PP_TRAMPOLINE(macro), CHAOS_PP_TYPEOF(g), g, __VA_ARGS__ \
        )) \
        /**/
...
#    define CHAOS_IP_FOR_EACH_I_PARAMETRIC_I(s, id, para, o, n, macro, _m, type, g, ...) \
        CHAOS_PP_IIF(CHAOS_PP_IS_CONS(g))( \
            CHAOS_IP_FOR_EACH_I_PARAMETRIC_II, CHAOS_PP_TUPLE_EAT(?) \
